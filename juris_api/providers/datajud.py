...
from juris_api.utils.text import (
    clean_url,
    extract_name_list,
    first_non_empty,
    infer_document_type,
...
class DataJudProvider(BaseProvider):
    ...
    async def search(self, services: ServicesContainer, settings: Settings, tribunal: str, query: str, limit: int) -> ProviderExecution:
        ...
        payload = {
            ...
            '_source': {
                'includes': [
                    'numeroProcesso', 'ementa', 'texto', 'textoIntegral',
                    'orgaoJulgador.nome', 'orgaoJulgador',
                    'classe.nome', 'classe',
                    'assuntos.nome', 'assuntos',
                    'relator.nome', 'relator', 'magistradoRelator.nome',
                    'dataJulgamento', 'dataPublicacao', 'dataHoraUltimaAtualizacao', 'dataAjuizamento',
                    'grau',
                    'movimentos.nome', 'movimentos.codigo', 'movimentos.complementosTabelados.nome',
                    'sistema.nome', 'sistema',
                    'tipoDocumento', 'tipoDecisao', 'documento.tipo',
                    'urlDocumento', 'urlInteiroTeor', 'link',
                ]
            },
            'query': {
                'multi_match': {
                    'query': query,
                    'fields': [
                        'numeroProcesso^8', 'ementa^6', 'texto^5', 'assuntos.nome^3', 'classe.nome^2',
                        'orgaoJulgador.nome^2', 'movimentos.nome', 'movimentos.complementosTabelados.nome'
                    ],
                    'type': 'best_fields',
                    'operator': 'or',
                }
        ...
        try:
            ...
            for hit in hits:
                ...
                movimentos = extract_name_list(src.get('movimentos'))
                numero = safe_text(src.get('numeroProcesso'), 80)
                ementa = safe_text(first_non_empty(src.get('ementa'), src.get('texto'), src.get('textoIntegral')), 5000)
                ...
                inteiro_teor_url = clean_url(first_non_empty(src.get('urlInteiroTeor'), src.get('urlDocumento'), src.get('link')))
                fonte_url = inteiro_teor_url or 'https://www.cnj.jus.br/sistemas/datajud/'
                sistema = normalize_source_system(first_non_empty(nested_get(src, 'sistema', 'nome'), src.get('sistema')))
                tipo_documento = infer_document_type(
                    src.get('tipoDocumento'),
                    src.get('tipoDecisao'),
                ...
                relevance = lexical_relevance(
                    query,
                    ' '.join(filter(None, [ementa or '', classe or '', orgao or '', relator or '', ' '.join(assuntos), ' '.join(movimentos)])),
                    upstream_score=float(hit.get('_score') or 0.0),
                )
                meta_score = metadata_quality(
                    ...
                    has_assunto=bool(assuntos),
                )
                score = final_score(
                    confidence,
                    relevance,
                    metadata_score=meta_score,
                    recency_score=recency_signal(data_julgamento or data_publicacao),
                ...
                fonte = FonteMetadata(
                    ...
                    oficial=True,
                    endpoint=url,
                    score_confiabilidade=confidence,
                    observacao='API pública oficial do CNJ.',
                )
                results.append(
                    ResultadoJuridico(
